...
var registration = {
    ...
    retrieveChannels: function () {
        ...
        var columns = [
            ...
            {
                field: "channelGroupID",
                title: "Channel Group",
                width: 250,
                filterable: false,
                template: function (dataItem) {
                    return dataItem.channelGroup && dataItem.channelGroup.name ? dataItem.channelGroup.name : "";
                }
            },
            {
                field: "minRange",
        ...
        // channel group event handler
        $('#channel_group').change(this.channel_group_Change);
        ...
        for (var i = 0; i < dropdownlist.length; i++) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (dropdownlist[i].ctrl === 'channel_chartType' || dropdownlist[i].ctrl === 'channel_group')
                        drop.prepend("<option value='' selected='selected'></option>");
                }
            });
        }
        ...
        $(registration.modules.channel.formId).bootstrapValidator({
            ...
            .on('success.form.bv', function (e) {
                ...
                var data = {
                    ...
                    channelGroupID: $('#channel_group').val() === '' ? null : $('#channel_group').val(),
                    chartTypeID: $('#channel_chartType').val() === '' ? null : $('#channel_chartType').val(),
    ...
    channel_group_Change: function () {
        $('#channel_chartType').val('');
        var url = Settings.WebApiUrl + "/api/KendoChannelGroups/" + $(this).val();
        $.ajax({
            url: url,
            type: "get",
            dataType: "json",
            ...
            success: function (result) {
                if (result.chartType && result.chartType.id) {
                    $('#channel_chartType').val(result.chartType.id);
                }
            }
        });
    },
    ...
    btnEditChannel_OnClick: function () {
        ...
        if (selectedItem) {
            ...
            $('#channel_group').val(selectedItem.channelGroupID);
