var index = {
    ...
    onload: function () {
        ...
        Util.displayLoading($("#dataChart"), true);
        setTimeout(function () {
        ...
        $.ajax({
            type: 'GET',
            url: Settings.WebApiUrl + 'api/KendoMonitorings/GetNotifications',
            dataType: 'json',
    ...
    createTimeLine: function (result) {
        var divTemplate =
            '<li {{invert}}>' +
            '    <div class="timeline-badge {{level}}">' +
        ...
        $.each(result, function (index) {
            var noti = result[index];
            var timelineRoot = $('#timeline');

            var level = 'success';
            var icon = 'fa-info-circle';
            var inverted = index % 2 === 0 ? '' : 'class="timeline-inverted"';
            ...
            divHtml = divTemplate;
            divHtml = divHtml.replace(/{{invert}}/g, inverted);
            divHtml = divHtml.replace(/{{level}}/g, level);
            divHtml = divHtml.replace(/{{level-icon}}/g, icon);
            divHtml = divHtml.replace(/{{header}}/g, noti.header);
            divHtml = divHtml.replace(/{{time}}/g, noti.timeStamp);
            divHtml = divHtml.replace(/{{body}}/g, noti.message);

            $(divHtml).appendTo(timelineRoot);
        });
    },
    ...
    createChart: function () {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                Util.displayLoading($("#dataChart"), false);
            }//end of success
        });
    },
